  ...
  nixpkgs.overlays = [
    ...
    (self: super: {
      ...
      flameshot-upload = self.writeScriptBin "flameshot-upload" ''
        ...
        ${self.curl}/bin/curl -H "$(${self.coreutils}/bin/cat /run/agenix/zipline-upload-headers)" https://holy.fuckk.lol/api/upload -F file=@/tmp/screenshot.png -H 'content-type: multipart/form-data' -H x-zipline-domain: furryfemboy.ca | ${self.jq}/bin/jq -r .files[0].url | tr -d '\n' | ${self.xclip}/bin/xclip -selection clipboard
      '';
    })
  ];
}